...
![Node photo placeholder](docs/images/hero.jpg)
> *Replace with a photo of the assembled node. A render is second-best. Do this before shipping — a wall of text without an image loses most readers in the first few seconds.*

> **⚠️ FIRMWARE IS NOT TESTED ⚠️**
> **⚠️ This repository documents the hardware only. Node firmware is in progress and unvalidated. ⚠️**
...
This repo covers the **hardware** — sensing, power, and enclosure. The application and server are built separately by my teammate for the Congressional App Challenge.
...
- **BME688** — *secondary* gas sensing plus temperature, humidity, and pressure. Its MOX element responds broadly to VOCs, which makes it a useful cross-check against the SGP41 rather than a replacement for it.
...
![Schematic placeholder](docs/images/schematic.png)
> *Export from KiCad and drop in here.*

**Battery wiring note:** the battery is wired directly to the Heltec's battery pads — **meter polarity against the silkscreen before connecting.** Heltec has shipped board revisions with the battery connector pinout reversed, and reversing it kills the board instantly.
...
| *SHT41* | *Purchased before design change — not used* | *1* | *$9.99* | ⚠️ *Cut* |
...
> **Note on the SHT41:** it was purchased when the design briefly dropped the SGP41. The SGP41 was reinstated because the SHT41 has no gas-sensing hardware — it is the same silicon as the SHT45 in a lower accuracy bin. It is listed here so the receipts reconcile against the parts list.
...
- **The enclosure is 3D printed, which is a known contamination risk.** Heated thermoplastic outgasses VOCs, and this node's primary sensor measures VOCs. PETG was chosen over PLA and ABS for lower residual emission, and the print is aired out before assembly, but a sealed-enclosure baseline run is required before any VOC reading can be trusted. CNC-machined parts were the original plan and remain the better answer.
...
- **Hardware, power system, enclosure, and node firmware:** Cai Griffith — *this repository*
- **Application and server backend:** *[teammate]* — *[link to team repo]*

CropSentry is a two-person project built for the **Congressional App Challenge**, and approved as a research project through **Middle Georgia State University**.

**--------------**

### 📺 *More from LayerForge Productions*

- **Portfolio:** [caigriffith.dev](https://caigriffith.dev)
- **Free 3D Printing & Engineering Course:** [forgecore.org](https://forgecore.org)
- **YouTube:** [@LayerforgeProductions](https://www.youtube.com/@LayerforgeProductions)

**--------------**

### 📄 *License*

*Add a LICENSE file. MIT is the usual default for hardware repos; CERN-OHL or TAPR are the hardware-specific options if the distinction matters to you.*
